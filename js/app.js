...
(function () {
  ...
  /* =========================================================
     ...
     ========================================================= */
  const STORAGE_KEYS = {
    balance: "bayardisini_balance",
    transactions: "bayardisini_transactions",
    theme: "bayardisini_theme",
    profile: "bayardisini_profile",
  };
  ...
  function renderPaymentMethodContent() {
    ...
    if (state.selectedMethod === "qris") {
      ...
      new QRCode(el("qrisCanvas"), {
        text: "BAYARDISINI|QRIS|" + p.nomor + "|" + p.amount + "|" + generateId(),
        width: 168,
        height: 168,
      });
  ...
  function renderReceipt(tx) {
    el("receiptContent").innerHTML =
      '<div class="receipt-brand"><strong>BAYAR DISINI</strong><br><small style="color:var(--muted)">Bukti Pembayaran Elektronik</small></div>' +
      ...
      '<div class="stamp-wrap"><div class="stamp"><span class="stamp-text">SUDAH<br>LUNAS</span></div></div>' +
  ...
  function personalizeGreeting(name) {
    const heading = el("dashboard-heading");
    if (!heading) return;
    const firstName = (name || "").trim().split(" ")[0];
    heading.textContent = firstName ? "Halo, " + firstName + " \uD83D\uDC4B" : "Selamat datang kembali";
  }
  ...
  function checkAuth() {
    ...
    if (saved && saved.name) {
      hideAuthScreen();
      personalizeGreeting(saved.name);
    } else {
      showAuthScreen();
    }
  }
  ...
  el("loginForm").addEventListener("submit", (e) => {
    e.preventDefault();
    const nameField = el("loginName");
    const emailField = el("loginEmail");
    const name = nameField.value.trim();
    const email = emailField.value.trim();
    let ok = true;

    if (!name) {
      el("loginNameError").hidden = false;
      nameField.classList.add("error");
      ok = false;
    } else {
      el("loginNameError").hidden = true;
      nameField.classList.remove("error");
    }

    if (!email || !isValidEmail(email)) {
      el("loginEmailError").hidden = false;
      emailField.classList.add("error");
      ok = false;
    } else {
      el("loginEmailError").hidden = true;
    ...
    showToast("Berhasil masuk. Selamat datang, " + name.split(" ")[0] + "!", "success");
  });
  ...
  el("logoutBtn").addEventListener("click", () => {
    ...
    el("loginForm").reset();
    showToast("Kamu sudah keluar.", "info");
    showAuthScreen();
  });
  ...
  /* =========================================================
     ...
     ========================================================= */
  const faqs = [
    { q: "Apakah aplikasi ini memproses uang sungguhan?", a: "Tidak. Bayar diSini adalah aplikasi simulasi untuk keperluan pembelajaran, seluruh saldo dan transaksi bersifat fiktif dan tersimpan di perangkat Anda sendiri (localStorage)." },
